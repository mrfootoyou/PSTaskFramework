...
# spell:ignore psargs,winget,choco,8wekyb3d8bbwe
...
function Install-PowerShellModule {
    ...
    function installIfNeeded([string]$Name, [version]$MinimumVersion, $Scope) {
        if (!($installed = tryGetModule @PSBoundParameters)) {
            Write-Information "Installing $Name $MinimumVersion (or greater) in $Scope scope..."
        ...
        Write-Information "$($PSStyle.Foreground.Green)$Name $($installed.Version) is installed."
    }
