...
Describe 'build-helpers' {
    BeforeAll {
        $modulePath = Join-Path $PSScriptRoot 'build-helpers.psm1'
        Import-Module $modulePath -Force -Scope Local
    }

    AfterAll {
        Remove-Module -Name build-helpers -ErrorAction Ignore
    }
    ...
    BeforeEach {
        ...
        Mock -CommandName 'Write-Host' -ModuleName build-helpers -MockWith { } -Verbose:$false
    }

    Context 'Assert-AppExists' {
        It 'returns first discovered application source path with PassThru' {
            Mock -CommandName 'Get-Command' -ModuleName build-helpers -MockWith {
                @(
            ...
            Should -Invoke -CommandName 'Get-Command' -ModuleName build-helpers -Times 1 -Exactly
        }

        It 'throws by default when app is missing' {
            Mock -CommandName 'Get-Command' -ModuleName build-helpers -MockWith { $null }
    ...
    Context 'Invoke-Shell' {
        It 'echoes command text and succeeds for zero exit code' {
            Mock -CommandName 'Assert-AppExists' -ModuleName build-helpers -MockWith { 'pwsh' }

            Invoke-Shell -- pwsh -NoLogo -NoProfile -Command 'exit 0'

            $global:LASTEXITCODE | Should -Be 0
            Should -Invoke -CommandName 'Write-Host' -ModuleName build-helpers -Times 1 -Exactly
        }
        ...
        It 'suppresses command echo when NoEcho is specified' {
            ...
            Should -Invoke -CommandName 'Write-Host' -ModuleName build-helpers -Times 0 -Exactly
        }
    ...
    Context 'Test-Administrator' {
        ...
        It 'returns true for root uid on non-Windows' -Skip:$IsWindows {
            Mock -CommandName 'getUserId' -ModuleName build-helpers -MockWith { '0' }

            Test-Administrator | Should -BeTrue
        }

        It 'returns false for non-root uid on non-Windows' -Skip:$IsWindows {
            Mock -CommandName 'getUserId' -ModuleName build-helpers -MockWith { '1000' }
